...
from ATTUDPServer import ATTUDPServer, ATTUDPSERVER_SUC, ATTUDPSERVER_FAIL
from robotremoteserver import RobotRemoteServer
from initapp import REMOTE_PORTS
import attlog as log

REMOTE_PORT = REMOTE_PORTS.get('UDPServer')
VERSION = '1.0.0'
REMOTE_TIMEOUT = 3600
...
class UDPServer():
    ...
    def switch_udp_server(self, alias):
        """
        功能描述：切换当前已初始化的UDP Server；
        ...
        try:
            ...
            if (isinstance(obj, Remote)):
                ...
                log_data = u'切换到别名为：%s 的UDPServer成功' % alias
    ...
    def start_udp_server(self, port=60000, ip='0.0.0.0'):
        """
        功能描述：开启UDP Server
        
        参数：
            port: UDP Server监听的端口号，默认为60000
            
            ip:   UDP server监听的IP地址,默认为0.0.0.0，监听主机上所有的地址
            
        Example:
        | Init Udp Server  | local  |             |
        | Start Udp Server | 60010  | 10.10.10.10 |
        """
        ...
        if (isinstance(obj, Remote)):
            ...
            ret, log_data = obj.start_udp_server(ip, int(port))
    ...
    def stop_all_udp_server(self):
        """
        功能描述：停止所有开启的 UDP Server
        ...
        if not alias_list:
            log.user_info(u"没有初始化UDP Server!")
