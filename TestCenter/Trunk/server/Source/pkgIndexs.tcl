...
# 
#  Description：main file of HLAPI, and used to reference all the files                                            
# 
#  Creator： David.Wu
#
#  Time:  2007.3.2
...
# support for different STCVersion, added by lana-20130706
set gSTCVersionList {3.60 3.70 3.80 3.90 3.95 4.00 4.20}
...
if [catch {
	...
	# 轮询遍历，检查当前版本是否在支持范围之内  changed by lana -20130706
	# 版本是否匹配的标志，为0：不匹配，为1：匹配 
	set versionMatch 0
	foreach version $gSTCVersionList {
		
		set gSTCVersion $version
	...
	if {$versionMatch == 1} {
		...
		puts "SpirentTestCenter version is not match.\nPlease ensure your SpirentTestCenter version is in \[3.60 3.70 3.80 3.90 3.95 4.00 4.20\]"
		return -1
	}
	
} errMsg ] {
    puts "Loading SpirentTestCenter Package failed : $errMsg\nPlease ensure SpirentTestCenter 3.95 is installed on this PC"
    return -1
}
...
#
#Input: StreamName: name of the stream  StreamResultHandle：stream的handle, type: stream type
#       TotalPageCount: total pages of stream result   hGeneratorStatus：status of the generator   
#
#
