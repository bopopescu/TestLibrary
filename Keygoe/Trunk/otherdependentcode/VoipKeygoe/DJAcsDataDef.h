...
typedef struct
{
	...
	DJ_U8			u8MediaMode;			// media mode 0:noset(sendrecv) 1:sendonly 2:recvonly 3:sendrecv
	DJ_S32			s32ReInvite;
}VoIPCallModifyPrivateEx_t;/*VoIPCallPrivate_t*/

typedef struct
{
	PrivateData_t   m_PrivateData;         /*privatedata head*/ 
	DJ_S32          m_s32Protocol;         /*protocol type*/
	DJ_S8			m_s8CallerTelNo[20];
	DJ_S8           m_s8CallerAddress[20]; /*Caller Address*/
	DJ_S32          m_s32CallerPort;       /*Caller Port*/
	DJ_S8           m_s8CallerUserID[40];  /*Caller UserID*/
	DJ_S8			m_s8CalleeTelNo[20];
	DJ_S8           m_s8CalleeAddress[20]; /*Callee Address*/
	DJ_S32          m_s32CalleePort;       /*Callee Port*/
	DJ_S8           m_s8CalleeUserID[40];  /*Callee UserID*/
	DJ_S8			m_s8BeOption;		// 1：表示添加发送Option
											// 非1：表示删除发送Option
	DJ_U8			m_u8Reserved[3];
}VoIPCallOptionPrivate_t;
...
typedef struct
{
	DJ_S8	m_s8WanAddress[40];		// proxy address
}Acs_VoIP_WANADDR;
...
typedef struct 
{
	...
	DJ_S8	m_s8TelNo[100];			/* Client Telephone-subscriber */
...
{
    DJ_U8	m_u8State;		/* Registration State */    
    DJ_S8	m_s8TelNo[100];	    	/* subscriber number */
...
/* Acs VoIP Call Transfer struct */
typedef struct 
{
	DJ_U8			m_u8SrcUnitId;			// source dsp unit id
	DJ_U8			m_u8DstUnitId;			// destination dsp unit id
	DJ_S16			m_u16DstCallId;			// destination Channel id
	DJ_S8			m_s8TelNo[20];
}Acs_VoIP_Transfer;/*Acs_VoIP_Transfer*/
...
typedef struct
{
	...
	DJ_S8	m_s8Username[40];	/* username for authorization */
	DJ_S8	m_s8Password[20];	/* paswword for authorization */
	DJ_S8	m_s8ProxyAddress[40];		// proxy address
	DJ_S8	m_s8UserID[40];			// user display name
	DJ_U16	m_u16ProxyPort;			// proxy port
	DJ_U16  m_u16Reserved;
}Acs_VoIP_SIPREGEX;/*Acs_VoIP_SIPREG*/
...
enum XMS_VOIP_IO_TYPE
{
	...
	XMS_IO_TYPE_VOIP_CALLMODIFYANSWER =18,
	XMS_IO_TYPE_VOIP_OPTION			= 19,
	XMS_IO_TYPE_VOIP_OPTIONNOTIFY	= 20,
};
...
/* Acs voc bargein configuration struct */
typedef struct
{
	DJ_U16  enable;   // bargeIn enable	
	DJ_U16  DTMFSignalCutOff;  // 1: DTMF and tone cut off, will be not as voc		
	DJ_U32  maxPersonSilenceTime; //真人摘机的最大静音时长，默认值:700ms	
	DJ_U32   silenceTimeout;  //静音超时时长，默认值:10000ms		
	DJ_U32  maxMusicVocTime;  //彩铃的最大语音时长, 默认值:800ms	
	DJ_U32   voiceTimeout;  //语音超时时长，默认值:10000ms
	DJ_U32  maxToneSilenceTime; //提示音的最大静音时长,默认值:200ms
	DJ_U16   getNormalEvent;   //  1 : 获取noraml事件，0:不获取noraml事件
	DJ_U16   u_8reserved;	
	DJ_U32   u_32reserved;
	
}Acs_Voc_Bargein_Config;/*Acs_Voc_Bargein_Config*/

/* Acs voc bargein event struct */
typedef struct
{
	DJ_U16 event;  //事件标识
	DJ_U16  normalSqNo; //voc和silence的normal事件的sequence number, 对语音系列进行一下排序
	DJ_U32  time;  //对应事件所持续的时长
	DJ_U32  aveEnergy; //事件的平均能量
	DJ_U32  u32reserved[2];
}Acs_Voc_Bargein_Event;/*Acs_Voc_Bargein_Config*/
